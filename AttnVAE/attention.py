...
class BahdanauAttention(nn.Module):
	...
	def __init__(self, hidden_size_enc, hidden_size_dec):
		...
		self.attn = nn.Linear((hidden_size_enc)+hidden_size_dec, hidden_size_dec)
	...
	def forward(self, encoder_outputs, hidden, mask):
		...
		# hidden [batch_sie, seq_len, dec_hid_dim]
		# encoder_outputs [batch-size, seq_len, enc_hid_dim*2]

		ener = torch.tanh(self.attn(torch.cat((hidden, encoder_outputs), dim=2)))
		# ener [batch_size, seq_len, dec_hid_dim]
		attention = self.v(ener).squeeze(2)
		...
		attn_vec = torch.bmm(attention.unsqueeze(1), encoder_outputs).permute(1,0,2)
		#[1, batch_size, enc_hid_dim]
		return attn_vec, attention
...
class LuongAttention(nn.Module):
	...
	def forward(self, 
		...
		# enc_masks -> [seq_length, batch_size]
		...
		if enc_mask is not None:
			enc_mask = enc_mask.unsqueeze(1) # -> [seq_len, 1, batch_size]
			enc_mask = torch.transpose(enc_mask, 0, 2) # [batch-size, 1, seq_len]
